/* Le code present dans cette feuille Javascript permet la creation d'un objet permettant a son tour la signature dans le canvas de facon multisupports
Il est a noter que : 
...
/* UNE CLASSE SIGNATURE POUR REGROUPER TOUTES NOS METHODES RELATIVES A LA SIGNATURE MULTISUPPORTS */
class Signature {
	constructor() {
	...
	getPosition(e) { // Une methode de la classe pour recuperer les coordonnees exactes du pointeur en fonction du type de l'event (touch or mouse)
		...
		if (typeE === "mousemove") {
			return {
				x: e.clientX - rect.left, // On soustrait, tres logiquement la valeur obtenue de clientX et clientY (fenetre), à celle obtenue avec la methode precedente pour obtenir l'emplacement exacte du point
	...
	reinitialiser() { // Une methode pour nettoyer le canvas, très utile et reutilisable selon plusieurs cas de figure
